...
import type {
  CheckinState, AppMode, Reserva, GuestData,
  PartialGuestData, StepId, CheckinNav, NavDirection, CheckinActions,
} from "@/types";
...
export function buildEmptyState(appMode: AppMode): CheckinState {
  return {
    appMode, reserva: null, knownGuest: null,
    numAdultos: 1, numMenores: 0, numPersonas: 1,
    guests: [{ esMenor: false, relacionesConAdultos: [] }],
    horaLlegada: "", observaciones: "",
    rgpdAcepted: false, legalPassed: false, hasMinorsFlag: false,
  };
}
 
function mergeGuests(prev: PartialGuestData[], total: number): PartialGuestData[] {
  return Array.from(
    { length: total },
...
export function checkinReducer(state: CheckinState, action: CheckinAction): CheckinState {
  switch (action.type) {
    case "SET_KNOWN_GUEST":
      return { ...state, knownGuest: action.guest, guests: [{ ...action.guest, esMenor: false }] };
    case "SET_RESERVA_TABLET":
      return {
        ...state, reserva: action.reserva,
        numPersonas: action.reserva.numHuespedes,
        numAdultos: action.reserva.numHuespedes, numMenores: 0,
        guests: mergeGuests([], action.reserva.numHuespedes),
      };
    case "SET_RESERVA":
      return { ...state, reserva: action.reserva, numPersonas: action.reserva.numHuespedes };
    case "SET_NUM_PERSONAS": {
      const newGuests = mergeGuests(state.guests ?? [], action.total);
      return {
        ...state, numPersonas: action.total, guests: newGuests,
    ...
    case "UPDATE_GUEST": {
      ...
      if (typeof finalValue === "string") finalValue = finalValue.replace(/\s+/g, " ").trim();
      ...
      if (action.key === "fechaNac" && typeof finalValue === "string") {
        ...
        if (parsed.isValid()) updated.esMenor = dayjs().diff(parsed, "years") < 18;
      }
    ...
    case "UPDATE_RELACION": {
      ...
      if (action.parentesco === "") {
        menor.relacionesConAdultos = rels.filter((r) => r.adultoIndex !== action.adultoIndex);
      } else {
        const idx = rels.findIndex((r) => r.adultoIndex === action.adultoIndex);
        if (idx >= 0) rels[idx] = { ...rels[idx], parentesco: action.parentesco };
        else rels.push({ adultoIndex: action.adultoIndex, parentesco: action.parentesco });
    ...
    case "SET_LEGAL_PASSED":    return { ...state, legalPassed: action.value };
    case "SET_HAS_MINORS_FLAG": return { ...state, hasMinorsFlag: action.value };
    case "RESTORE_FULL_STATE":  return { ...state, ...action.payload };
    case "RESET":               return buildEmptyState(state.appMode);
    default:                    return state;
  }
}
 
interface HistoryEntry { step: StepId; guestIndex: number }
...
export function useCheckin(tokenUrl?: string, stepUrl?: string) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (cancelled) return;
        if (result.knownGuest) dispatch({ type: "SET_KNOWN_GUEST", guest: result.knownGuest });
        dispatch({ type: "SET_RESERVA", reserva: result.reserva });
        sessionStorage.setItem(`bookingId_${token}`, String(result.bookingId));
        if (result.clientId) sessionStorage.setItem(`clientId_${token}`, String(result.clientId));
      } catch (err) {
        if (cancelled) return;
  ...
  const goTo = useCallback(
    (nextStep: StepId, dir: NavDirection = "forward", gIdx?: number) => {
      ...
      setNavDirection(dir);
      setAppHistory((prev) => [...prev, { step: nextStep, guestIndex: gIdx ?? activeGuestIndex }]);
  ...
  const nextGuest = useCallback(
    (currIdx: number, from: StepId) => {
      ...
      if (from === "form_personal") {
        ...
        if (currIdx + 1 < numPersonas) return goTo("form_personal", "forward", currIdx + 1);
      }
      if (from === "form_contacto" && numPersonas > 1) return goTo("form_personal", "forward", 1);
      if (["form_personal","form_contacto","form_relaciones"].includes(from)) {
        const nextM = from === "form_relaciones"
  ...
  const canGoBack = appHistory.length > 0 && !["exito","tablet_buscar","inicio"].includes(actualStep);
  ...
  const nav: CheckinNav = useMemo(
    () => ({ step: actualStep, guestIndex: activeGuestIndex, direction: navDirection,
             dotSteps, dotIndex: currentDotIndex, canGoBack, allowedSteps, isNavigating }),
  ...
  const actions: CheckinActions = useMemo(
    () => ({
      ...
      updateGuest:       (index, key, value) => dispatch({ type: "UPDATE_GUEST", index, key, value }),
    ...
    [goTo, goBack, dispatch, dotSteps, currentDotIndex, nextGuest, activeGuestIndex]
  );
