...
const FieldError: React.FC<{ msg?: string }> = ({ msg }) =>
  msg ? (
    <span role="alert" aria-live="polite" className="field-err" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
...
export const ScreenFormPersonal: React.FC<FormPersonalProps> = ({
  ...
  const esMenor = fechaNac && fechaNac.isValid() ? dayjs().diff(fechaNac, 'years') < 18 : false;

  const handleNext = () => { if (validate(data)) onNext(); };
  ...
  return (
    <>
      ...
      <Box sx={{ p: '0 var(--px)', mt: 2, display: 'flex', flexDirection: 'column', gap: 2.5 }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
          <div>
            <TextField
              ...
              error={!!errors.nombre}
              sx={inputSx}
              inputProps={{ 'aria-describedby': errors.nombre ? 'err-nombre' : undefined }}
            />
          ...
          <div>
            <TextField
              ...
              sx={inputSx}
              inputProps={{ 'aria-describedby': errors.apellido ? 'err-apellido' : undefined }}
            />
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
          ...
          <div>
            ...
            <DatePicker
              ...
              onChange={(v) => onChange('fechaNac', v?.isValid() ? v.format('YYYY-MM-DD') : '')}
              slotProps={{
                textField: {
                  fullWidth: true,
                  error: !!errors.fechaNac,
                  sx: inputSx,
                  inputProps: { 'aria-describedby': errors.fechaNac ? 'err-fecha' : undefined },
                },
              }}
            />
        ...
        {isMainGuest && esMenor && (
          <Alert variant="err" style={{ margin: 0 }}>
            <strong>Acompañante adulto obligatorio.</strong>{' '}
            Indique abajo el nombre del responsable del menor.
          </Alert>
        )}
        ...
        {esMenor && isMainGuest && (
          <Box sx={{
            ...
            gap: 2, p: 2,
            bgcolor: 'var(--primary-lt)', borderRadius: '12px',
          }}>
            <div>
              <TextField
      ...
      <div className="btn-row">
        <Button onClick={handleNext} iconRight="right">Continuar</Button>
      </div>
    </>
  );
};
...
export const ScreenFormContacto: React.FC<FormContactoProps> = ({ data, onChange, onNext }) => {
  ...
  return (
    <>
      ...
      <Box sx={{ p: '0 var(--px)', mt: 2, display: 'flex', flexDirection: 'column', gap: 2.5 }}>
        <div>
          <TextField label="Email" required fullWidth value={data.email ?? ''} onChange={e => onChange('email', e.target.value)} error={!!errors.email} sx={inputSx} />
          <FieldError msg={errors.email} />
        </div>
        <div>
          <TextField label="Teléfono" required fullWidth value={data.telefono ?? ''} onChange={e => onChange('telefono', e.target.value)} error={!!errors.telefono} sx={inputSx} />
          <FieldError msg={errors.telefono} />
        </div>
        <TextField label="Dirección habitual" fullWidth value={data.direccion ?? ''} onChange={e => onChange('direccion', e.target.value)} sx={inputSx} />
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
          <TextField label="Ciudad" fullWidth value={data.ciudad ?? ''} onChange={e => onChange('ciudad', e.target.value)} sx={inputSx} />
          <div>
            <TextField select label="País" required fullWidth value={data.pais ?? ''} onChange={e => onChange('pais', e.target.value)} error={!!errors.pais} sx={inputSx}>
  ...
  return (
    <>
      ...
      <Box sx={{ p: '0 var(--px)', mt: 2, display: 'flex', flexDirection: 'column', gap: 2.5 }}>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
          <div>
            <TextField select label="Tipo de documento" required fullWidth value={data.tipoDoc ?? ''} onChange={e => onChange('tipoDoc', e.target.value)} error={!!errors.tipoDoc} sx={inputSx}>
              {TIPOS_DOCUMENTO.map(t => <MenuItem key={t} value={t}>{t}</MenuItem>)}
            </TextField>
            <FieldError msg={errors.tipoDoc} />
          </div>
          <div>
            <TextField label="Número" required fullWidth value={data.numDoc ?? ''} onChange={e => onChange('numDoc', e.target.value)} error={!!errors.numDoc} sx={inputSx} />
