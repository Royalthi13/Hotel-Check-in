            ...
            <React.Fragment key={idx}>
              <ConfirmBlock
                ...
                rows={((): Array<[string, string | undefined | null]> => [
                  ...
                  [t("forms.gender"),    g.sexo ? t(`constants.sexos.${g.sexo}`) : null],
                  [t("forms.birthdate_clean"), g.fechaNac ?? null],
                  [
                    t("forms.nationality"),
                    g.nacionalidad ? t(`constants.nacionalidades.${g.nacionalidad}`) : null,
                  ],
              ...
              <ConfirmBlock
                ...
                onEdit={() => onEditStep("form_personal", idx)}
                rows={[
                  [t("forms.doc_type"),   g.tipoDoc ? t(`constants.documentos.${g.tipoDoc}`) : null],
                  [t("forms.doc_number"), g.numDoc ?? null],
                  ...(g.vat ? [[t("forms.vat"), g.vat] as [string, string]] : []),
                  [t("forms.photo"), g.docUploaded ? t("review.photo_attached") : "—"],
                ]}
              />
            </React.Fragment>
          ))}
...
export const ScreenExito: React.FC<ExitoProps> = ({ state, onAddHora, isPartial }) => {
  const { t } = useTranslation();
    ...
    <>
      <div className="success-wrap">
        ...
        <h1 className="success-title">
          {t("success.title_precheckin_1")}<br />{t("success.title_precheckin_2")}
        </h1>
        ...
        <div className="success-info-card">
          ...
          {horaLlegada && horaLlegada !== "No especificada" && (
            <div className="si-row">
              ...
              <span>{horaLlegada.includes(":") ? horaLlegada : t(`constants.horas.${horaLlegada}`)}</span>
            </div>
          )}
        </div>
